...
New-Item -Path (Join-Path -Path $script_dir -ChildPath "package/debug") -ItemType Directory -Force
Copy-Item -Path (Join-Path -Path $script_dir -ChildPath ".\Source\bin\Debug\net7.0\*.dll") -Destination (Join-Path -Path $script_dir -ChildPath "bin") -Force -Recurse
...
Compress-Archive -Path $Files -DestinationPath (Join-Path -Path $script_dir -ChildPath "package/debug/CelesteTASLobbyWarpHelperD.zip") -Force